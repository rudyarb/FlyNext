...
export async function GET(request: Request): Promise<Response> {
  ...
  try {
    ...
    const flightsInfo: FlightInfo[] = flights.map((flight) => ({
    ...
    const hotelsInfo: HotelInfo[] = hotels.map((hotel: any) => ({
      hotelId: hotel?.hotelId ?? "N/A",
      roomTypeId: hotel?.roomTypeId ?? "N/A",
      bookingDate: hotel?.bookingDate ?? "N/A",
      checkInDate: hotel?.checkInDate ?? "N/A",
      checkOutDate: hotel?.checkOutDate ?? "N/A",
